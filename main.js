...
// Scope everything to stop users from touching internal variables.
(function() {
    ...
    var videoCrank = new QualityControl(document.getElementById("videoQualityControl"), videoQualityCheck);
    var audioCrank = new QualityControl(document.getElementById("audioQualityControl"), audioQualityCheck);
    ...
    // Controller support
    // Index of -1 indicates no controller connected
    var controller = -1;
    // vqc: Video Quality Crank
    // aqc: Audio Quality Crank
    var ventBtnPressed = false;

    function videoQualityCheck(obj, time) {
        // If a controller is connected, read its state
        if (controller !== -1) {
            var controllerObj = navigator.getGamepads()[controller];
            // Add a small dead zone.
            if (typeof(controllerObj) !== "undefined" && controllerObj.timestamp !== obj.gamepadState.timestamp && 
                (Math.abs(controllerObj.axes[2]) >= 0.05 || Math.abs(controllerObj.axes[3]) >= 0.05) &&
                (controllerObj.axes[2] !== obj.gamepadState.x || controllerObj.axes[3] !== obj.gamepadState.y)) {
                obj.gamepadState.x = controllerObj.axes[2];
                obj.gamepadState.y = controllerObj.axes[3];
                obj.gamepadState.timestamp = controllerObj.timestamp;
                if (obj.active) {
                    obj.move(obj.gamepadState.x, obj.gamepadState.y);
                } else {
                    obj.start(obj.gamepadState.x, obj.gamepadState.y);
                    obj.gamepadState.active = true
                }
            } else if (obj.gamepadState.active) {
                obj.end(true);
                obj.gamepadState.active = false;
            }
        }
        if (playing) {
            if (videoQuality === 0) {
    ...
    function audioQualityCheck(obj, time) {
        ...
        if (controller !== -1) {
            ...
            // Add a small dead zone.
            if (typeof(controllerObj) !== "undefined" && controllerObj.timestamp !== obj.gamepadState.timestamp && 
                (Math.abs(controllerObj.axes[0]) >= 0.05 || Math.abs(controllerObj.axes[1]) >= 0.05) &&
                (controllerObj.axes[0] !== obj.gamepadState.x || controllerObj.axes[1] !== obj.gamepadState.y)) {
                obj.gamepadState.x = controllerObj.axes[0];
                obj.gamepadState.y = controllerObj.axes[1];
                obj.gamepadState.timestamp = controllerObj.timestamp;
                if (obj.active) {
                    obj.move(obj.gamepadState.x, obj.gamepadState.y);
                } else {
        ...
        if (now >= nextPressureFrame) {
            ...
            if (controller !== -1) {
                ...
                if (typeof(controllerObj) !== "undefined") {
                    if (controllerObj.buttons[0].pressed) {
                        if (!pressureVenting) {
    ...
    function backgroundTask() {
        ...
        if (videoCrank.moved || audioCrank.moved) {
    ...
    function load(e) {
        ...
        if (videoUrl.match(/^(https\:\/\/)?twitch\.tv|^(https\:\/\/)?m\.twitch\.tv|^(https\:\/\/)?www\.twitch\.tv/gi) !== null) {
            ...
            if (remove_after != -1) {
                videoid = videoid.substr(0, remove_after);
            }
    ...
    window.addEventListener("gamepadconnected", function(e) {
        if (e.gamepad.axes.length >= 4) {
            ...
            videoCrank.gamepadState.x = e.gamepad.axes[2];
            videoCrank.gamepadState.y = e.gamepad.axes[3];
            videoCrank.gamepadState.timestamp = e.timeStamp;
            videoCrank.gamepadState.active = false;
            audioCrank.gamepadState.x = e.gamepad.axes[0];
            audioCrank.gamepadState.y = e.gamepad.axes[1];
            audioCrank.gamepadState.timestamp = e.timeStamp;
            audioCrank.gamepadState.active = false;
        }
    });

    window.addEventListener("gamepaddisconnected", function(e) {
        if (e.gamepad.index === controller) {
            controller = -1;
            if (videoCrank.gamepadState.active) {
                videoCrank.end(true);
            }
            if (audioCrank.gamepadState.active) {
                audioCrank.end(true);
            }
        }
    }); 
